...
let myStrongest = monstersInYourPocket.filter(({ monster, CP }) => CP > 200);
...
// let ordersTotal = orders.reduce(({price,tax},total) => total+price*tax);
let ordersTotal = orders.map(({ price, tax }) => price * (1 + tax));
...
// console.log(purchases.filter((purchase) => purchase.owner === "Bob"))//.reduce((total, {price}) => total+price))
